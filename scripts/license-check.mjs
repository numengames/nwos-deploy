...
const ALLOWED = new Set(["MIT", "ISC", "BSD-2-CLAUSE", "BSD-3-CLAUSE", "BSD", "APACHE-2.0", "0BSD", "CC0-1.0", "CC-BY-4.0", "OFL-1.1"]);
const ISOLATION_ONLY = /^(MPL-2\.0|EPL-2\.0|LGPL-3\.0(-only|-or-later)?)$/i;
const COPYLEFT_SEPARATE_REPO = /^(A?GPL)-[0-9.]+(-only|-or-later)?$/i;
const NEVER = /BUSL|SSPL|ELASTIC|COMMONS.?CLAUSE|-NC(-|$)|-ND(-|$)|UNLICENSED|PROPRIETARY/i;
...
export function licenceProblem(expr) {
	...
	if (COPYLEFT_SEPARATE_REPO.test(bare)) return `copyleft fuerte de terceros (${bare}): solo en repo separado con ` + `decisión firmada; además el artefacto es AGPL — sin excepción posible`;
	if (ISOLATION_ONLY.test(bare)) return `licencia de aislamiento (${bare}) dentro del bundle: empaquetado ` + `junto al resto no hay aislamiento`;
...
export function parseLegalDebt(markdown) {
	...
	for (const line of markdown.split("\n")) {
		...
		if (cells.length < 5 || cells[1] === "Package" || /^-+$/.test(cells[1])) continue;
		if (!cells[1]) continue;
...
function main() {
	...
	if (maps.length === 0) {
		errors.push("el build no emitió sourcemaps: sin ellos la guardia no puede inspeccionar " + "el contenido del artefacto (revisa vite.build.sourcemap en astro.config.mjs)");
	}
	...
	const legalDebt = fs.existsSync(debtPath) ? parseLegalDebt(fs.readFileSync(debtPath, "utf-8")) : new Map();
	...
	for (const [pkg, files] of [...shipped.entries()].sort()) {
		...
		if (debt && debt.kind !== "terms-verified") {
			errors.push(`${pkg}: umbral de LEGAL_DEBT.md incumplido — el paquete aparece en el ` + `artefacto (${where}); la excepción solo valía mientras no se distribuyera`);
			continue;
		}

		const licence = resolveLicence(pkg) ?? debt?.licence ?? null;
		if (!licence) {
			errors.push(`${pkg}: sin campo license resoluble (${where}); lee su LICENSE y ` + `documenta los términos en LEGAL_DEBT.md (kind terms-verified) o retíralo`);
			continue;
		}
	...
	if (fontFiles.length > 0) {
		const oflShipped = distFiles.some((f) => /\.(txt|md)$/i.test(f) && fs.readFileSync(f, "utf-8").includes("SIL OPEN FONT LICENSE Version 1.1"));
		if (!oflShipped) {
			errors.push(`el artefacto distribuye ${fontFiles.length} archivo(s) de fuente pero ` + `ningún texto OFL-1.1 los acompaña (esperado: public/licenses/OFL-1.1.txt → dist/)`);
		}
	}

	// --- Veredicto ----------------------------------------------------------

	console.log(`license-check: ${shipped.size} paquetes en el artefacto, ` + `${maps.length} sourcemaps, ${fontFiles.length} fuentes, ` + `${legalDebt.size} entradas en LEGAL_DEBT.md`);
	if (errors.length > 0) {
