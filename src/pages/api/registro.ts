...
/** The body the deploy form posts. Every field is validated before use. */
...
async function commitFile(octokit: Octokit, org: string, repo: string, path: string, content: string, message: string) {
	...
	await octokit.request("PUT /repos/{owner}/{repo}/contents/{path}", {
		...
		sha: (fileData as GitHubContent).sha,
	});
}
...
export const POST: APIRoute = async ({ request, locals }) => {
	try {
		...
		try {
			const { data: reuseData } = await octokit.request("GET /repos/{owner}/{repo}/contents/{path}", { owner: org, repo: slug, path: "REUSE.toml" });
			mouldSpec = parseMouldSpec(Buffer.from((reuseData as GitHubContent).content ?? "", "base64").toString("utf-8"));
			if (!mouldSpec) {
				log.error("deploy.abort", {
					repo: `${org}/${slug}`,
					reason: "REUSE.toml carries no parseable mould spec",
				});
			}
		} catch (e) {
			log.error("deploy.abort", {
				repo: `${org}/${slug}`,
				reason: "cannot read REUSE.toml from the generated repo",
		...
		// catch del bucle).
		const filesToPersonalize = [mouldSpec.renameFrom, "README.md", "CHANGELOG.md", "web/index.html", "canon/C-001-mission-vision-values.md", "canon/C-002-culture.md", "canon/C-003-org-structure.md", "canon/C-004-glossary.md"];

		for (const filePath of filesToPersonalize) {
			try {
				const { data: fileData } = await octokit.request("GET /repos/{owner}/{repo}/contents/{path}", { owner: org, repo: slug, path: filePath });

				const content = Buffer.from((fileData as GitHubContent).content ?? "", "base64").toString("utf-8");
				...
				if (filePath === mouldSpec.renameFrom) {
					log.error("deploy.abort", {
						repo: `${org}/${slug}`,
						reason: `${mouldSpec.renameFrom} missing or personalization failed`,
						error: errorMessage(e),
					});
					return new Response(
						JSON.stringify({
		...
		try {
			...
			const { data: licenseData } = await octokit.request("GET /repos/{owner}/{repo}/contents/{path}", { owner: org, repo: slug, path: mouldSpec.renameFrom });
			const licenseContent = Buffer.from((licenseData as GitHubContent).content ?? "", "base64").toString("utf-8");
			...
			log.error("deploy.abort", {
				repo: `${org}/${slug}`,
				reason: "client LICENSE install failed",
				error: errorMessage(e),
			});
			return new Response(
				JSON.stringify({
		...
		try {
			...
			log.error("deploy.status.failed", {
				repo: `${org}/${slug}`,
				error: errorMessage(e),
			});
		}
